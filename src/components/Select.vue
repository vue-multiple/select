    ...
    <div
        ...
        <div
            ...
            <transition-group @after-leave="resetInputHeight">
                <vm-tag
                        ...
                        close-transition>
                    <span class="vm-select__tags-text">{{ item.currentLabel }}</span>
                </vm-tag>
            </transition-group>

            <input
                    type="text"
                    class="vm-select__input"
                    @focus="visible = true"
                    :disabled="disabled"
                    ...
                    v-if="filterable"
                    :style="{ width: inputLength + 'px', 'max-width': inputWidth - 42 + 'px', height: inputheight + 'px'}"
                    ref="input">
        </div>
        <vm-input
                ref="reference"
                ...
                :name="name"
                :inputclass="`${inputclass} vm-select-input__inner`"
                :disabled="disabled"
        ...
        <transition
            ...
            <vm-select-menu
                ...
                <p class="vm-select-dropdown__empty" v-if="emptyText && (allowCreate && options.length === 0 || !allowCreate)">{{ emptyText }}</p>
            </vm-select-menu>
        </transition>
    </div>
</template>
...
<script type="text/babel">
    ...
    import VmSelectMenu from './select-dropdown.vue';
    import VmOption from './option.vue';
    ...
    export default {
        ...
        watch: {
            ...
            value(val) {
                ...
                this.dispatch('VmFormItem', 'el.form.change', val);
            },

            query(val) {
                this.$nextTick(() => {
                    if (this.visible) this.broadcast('VmSelectDropdown', 'updatePopper');
                });
                this.hoverIndex = -1;
                ...
                if (this.remote && typeof this.remoteMethod === 'function') {
                    ...
                    this.broadcast('VmOption', 'resetIndex');
                } else if (typeof this.filterMethod === 'function') {
                    this.filterMethod(val);
                    this.broadcast('VmOptionGroup', 'queryChange');
                } else {
                    this.filteredOptionsCount = this.optionsCount;
                    this.broadcast('VmOption', 'queryChange', val);
                    this.broadcast('VmOptionGroup', 'queryChange');
                }
            ...
            visible(val) {
                if (!val) {
                    ...
                    this.handleIconHide();
                    this.broadcast('VmSelectDropdown', 'destroyPopper');
                    if (this.$refs.input) {
                    ...
                    this.handleIconShow();
                    this.broadcast('VmSelectDropdown', 'updatePopper');
                    if (this.filterable) {
                        ...
                        if (this.multiple) {
                            ...
                            if (!this.remote) {
                                this.broadcast('VmOption', 'queryChange', '');
                                this.broadcast('VmOptionGroup', 'queryChange');
                            }
                            this.broadcast('VmInput', 'inputSelect');
                        }
                    }
                }
        ...
        methods: {
            handleIconHide() {
                let icon = this.$el.querySelector('.vm-input__icon');
                if (icon) {
                    removeClass(icon, 'reverse');
                }
            },

            handleIconShow() {
                let icon = this.$el.querySelector('.vm-input__icon');
                if (icon && !hasClass(icon, 'vm-icon-circle-close')) {
                    addClass(icon, 'reverse');
                }
            },

            handleMenuEnter() {
                if (!this.dropdownUl) {
                    this.dropdownUl = this.$refs.popper.$el.querySelector('.vm-select-dropdown__wrap');
                    this.getOverflows();
                }
            ...
            resetInputHeight() {
                this.$nextTick(() => {
                    ...
                    input.style.height = Math.max(this.$refs.tags.clientHeight + 6, this.inputheight||36) + 'px';
